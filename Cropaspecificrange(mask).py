...
model = YOLO("parkinggrid.pt")  # 载入模型
results = model("result.png")  # 将car.jpg丢进result
img = cv2.imread("result.png")  # 将car.jpg载入cv2，并存储为img
...
for result in results:
    ...
    if len(boxes) > 0:  # 只保存一个边界框
        ...
        # 打印边界框坐标
        cv2.imwrite("cropped.png", crop)  # 将CROP存为JPG文件的图像
